...
echo "\\begin{longtable}{|c|c|c|c|c|c|c|c|}" >> $OUTPUT_FILE
echo "\\hline" >> $OUTPUT_FILE
echo "\\textbf{trestbps} & \\textbf{chol} & \\textbf{fbs} & \\textbf{restecg} & \\textbf{thalach} & \\textbf{exang} & \\textbf{oldpeak} & \\textbf{slope}\\\\" >> $OUTPUT_FILE
echo "\\hline" >> $OUTPUT_FILE



while IFS=, read -r trestbps chol fbs restecg thalach exang oldpeak slope ca thal target; do
  echo "$trestbps & $chol & $fbs & $restecg & $thalach & $exang & $oldpeak & $slope \\\\" >> $OUTPUT_FILE
  echo "\\hline" >> $OUTPUT_FILE
done < $INPUT_FILE
